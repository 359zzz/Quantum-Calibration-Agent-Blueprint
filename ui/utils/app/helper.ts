...
export const processIntermediateMessage = (
    ...
    const replaceMessage = (steps: IntermediateStep[]): boolean => {
        for (let i = 0; i < steps.length; i++) {
            // Match by ID if:
            // 1. The existing step is "in_progress" (e.g., "Awaiting Approval" placeholder)
            // 2. OR both ID and name match (same step being updated)
            const idMatches = steps[i].id === newMessage.id;
            const existingIsInProgress = steps[i].status === 'in_progress';
            const nameMatches = steps[i].content?.name === newMessage.content?.name;

            if (idMatches && (existingIsInProgress || nameMatches)) {
