...
// LangGraph processors for deepagents integration
const {
  processLangGraphStream,
  transformChatRequest,
} = require('./langgraph-processors');

// LangGraph configuration
const LANGGRAPH_ASSISTANT_ID = process.env.LANGGRAPH_ASSISTANT_ID || 'agent';
const USE_LANGGRAPH = process.env.USE_LANGGRAPH === 'true'; // Default to false

// --- Configuration ---
const NAT_BACKEND_URL = process.env.NAT_BACKEND_URL || 'http://127.0.0.1:8000';
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname.startsWith(HTTP_PROXY_PATH + '/')) {
    ...
    const isChatResume = backendPath === CORE_ROUTES.CHAT_RESUME;
    ...
    // Use LangGraph for chat endpoints when enabled
    if (USE_LANGGRAPH && (isChatStream || isChat)) {
      const doLangGraphChat = async () => {
        try {
          const body = await readRequestBody(req);
          const langGraphBody = transformChatRequest(
            body.toString(),
            LANGGRAPH_ASSISTANT_ID
          );

          // LangGraph endpoint: POST /runs/stream
          const langGraphUrl = `${UPSTREAM_HTTP_ORIGIN}/runs/stream`;

          const backendRes = await fetch(langGraphUrl, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
          ...
          await processLangGraphStream(backendRes, res);
        } catch (err) {
          console.error('[LangGraph] Request failed:', err.message);
          if (!res.headersSent) {
            res.writeHead(502, { 'Content-Type': 'application/json' });
          }
          res.end(
      ...
      return void doLangGraphChat();
    }

    // Fallback to original processors if LangGraph disabled
    if (isChatStream) return void doFetchAndProcess(processChatStream);
    if (isChatResume) return void doFetchAndProcess(processChatStream);
