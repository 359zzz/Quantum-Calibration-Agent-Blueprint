...
const Home = (props: any) => {
  ...
  const {
    state: { folders, conversations, selectedConversation, enableStreamingRagVizOptions, viewMode },
    dispatch,
  } = contextValue;
  ...
  const handleSelectConversation = (conversation: Conversation) => {
    // Clear thread_id when switching conversations (deepagents memory)
    ...
    dispatch({ field: 'viewMode', value: 'chat' });
    sessionStorage.setItem('viewMode', 'chat');
  ...
  const handleNewConversation = () => {
    // Clear thread_id for new conversation (deepagents memory reset)
    sessionStorage.removeItem('currentThreadId');
        ...
        >
          ...
          <div className="flex h-full w-full sm:pt-0">
            ...
            <main className="flex flex-col w-full min-w-0 max-w-full overflow-x-hidden pt-0 relative border-l md:pt-0 dark:border-white/20 transition-width">
              {viewMode === 'chat' && (
                <div className="flex flex-1 flex-col min-h-0 min-w-0 max-w-full dark:bg-black">
