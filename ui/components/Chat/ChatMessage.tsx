...
export const ChatMessage: FC<Props> = memo(
  ({ message, messageIndex, onEdit }) => {
    ...
    const isLastMessage = selectedConversation?.messages
      ? messageIndex === selectedConversation.messages.length - 1
      : false;
    const isAssistantStreaming = message.role === 'assistant' && messageIsStreaming && isLastMessage;

    const { displayedText, isRevealing } = useTypewriter(
      message.content || '',
      isAssistantStreaming,
      3 // characters per frame
    );
    ...
    const prepareContent = ({
      ...
      // Use contentOverride if provided (for typewriter effect)
      const effectiveContent = contentOverride !== undefined ? contentOverride : content;

      if (role === 'user') return effectiveContent.trim();
                  ...
                  <div className="overflow-x-auto prose dark:prose-invert flex-1 w-full flex-grow max-w-full whitespace-normal">
                    ...
                    >
                      {prepareContent({
                        ...
                        intermediateStepsContent: false,
                        responseContent: true,
                        contentOverride: isAssistantStreaming ? displayedText : undefined,
                      })}
                    </MemoizedReactMarkdown>
                    {/* Blinking cursor during typewriter effect */}
                    {isRevealing && (
                      <span className="inline-block text-[#76b900] animate-blink ml-0.5">▍</span>
                    )}
                  </div>
